# Specification: Scaffold Integrated Math 1 Course Application

## Overview

Scaffold the `apps/integrated-math-1/` application as a minimal runnable shell. The app already has `IM1.md` (curriculum table of contents) and `public/im-1-hero.png` (hero image). The design system is identical to Integrated Math 2 (warm academic orange, Lora/DM Sans/Fira Code, oklch tokens). This track creates the full project skeleton: config files, Convex schema, design system tokens, auth wiring, layout, landing page, and seed data so the app boots and renders pages.

## Functional Requirements

### FR-1: Project Configuration
- Copy and adapt all config files from IM2: package.json (with @math-platform/* workspace deps), tsconfig.json, vite.config.ts, vitest.config.ts, vitest.setup.ts, eslint.config.mjs, postcss.config.mjs, tailwind.config.ts, next.config.ts, wrangler.jsonc, .gitignore, .env.local template
- App name: `integrated-math-1`, Convex project: Integrated Math 1
...
- Copy IM2's globals.css verbatim (same warm academic orange theme):
  - Brand orange primary (#B14C00), complementary teal accent (#00828C)
...
- Copy IM2's tailwind.config.ts (same design tokens)

### FR-4: Layout and Providers
- Root layout.tsx with ConvexClientProvider, AuthProvider, ThemeProvider
- Header and footer components with Integrated Math 1 branding
- Middleware for auth-gated routes (component-approval pattern from IM3)

### FR-5: Landing Page
- Integrated Math 1 branded landing page with:
  - Hero section with course title, description, and im-1-hero.png
  - 14-module overview cards (grouped by subject area: algebra, functions, geometry)
...
- /curriculum route showing 14 modules with lesson listings
- Sourced from IM1.md content structure
...
- Seed script (convex/seed.ts) with:
  ...
  - 14 units with lesson placeholders (slugs and titles from IM1.md)
...
2. Landing page shows Integrated Math 1 branding with warm academic orange design
3. /auth/login renders login form
4. /curriculum shows 14 modules with lesson titles
5. /student/dashboard and /teacher/dashboard render placeholder pages
6. Convex schema matches IM3 schema (all tables present)
7. Seed script creates 14 units with ~99 lesson placeholders
8. `npm run lint && npm run build && npx tsc --noEmit` all pass
9. Root package.json has workspace scripts for integrated-math-1
