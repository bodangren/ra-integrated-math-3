...
- [x] Task: Copy Convex schema and configuration
    ...
    - [x] Copy schema.ts from IM3 verbatim (same schema as all apps)
...
- [x] Task: Implement DESIGN.md tokens in globals.css
    - [x] Copy IM2's globals.css verbatim (warm academic orange color palette)
    - [x] Verify oklch-based CSS custom properties for light and dark modes
    - [x] Verify Lora/DM Sans/Fira Code font stack
    - [x] Verify component base styles (buttons, cards, inputs, navigation)
    - [x] Verify animation utilities (fade-up, gradients)
- [x] Task: Configure tailwind.config.ts for DESIGN.md system
    - [x] Copy IM2's tailwind.config.ts (same oklch color tokens, radius, fonts)
...
- [x] Task: Create header and footer components
    - [x] Write components/header-simple.tsx with Integrated Math 1 branding
...
- [x] Task: Create landing page
    - [x] Write app/page.tsx with IM1 hero (using im-1-hero.png), 14-module overview cards, CTAs
...
- [x] Task: Create curriculum page
    - [x] Write app/curriculum/page.tsx showing 14 modules with lesson listings
...
- [x] Task: Write page tests
    ...
    - [x] Test curriculum page renders modules
- [x] Task: Measure - Phase 5 Manual Verification (Protocol in workflow.md)

## Phase 6: Seed Data and Verification

- [x] Task: Create seed data for Integrated Math 1
    - [x] Write convex/seed/units.ts — 14 units with lesson slugs and titles from IM1.md
...
- [x] Task: Update root workspace configuration
    - [x] Add ws:im1:lint, ws:im1:test, ws:im1:typecheck, ws:im1:build scripts to root package.json
- [x] Task: Final verification
    ...
    - [x] Run npm run test — PASS (7/7 tests)
